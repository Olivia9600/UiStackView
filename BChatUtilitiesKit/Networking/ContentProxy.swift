...
public class ContentProxy: NSObject {
    ...
    // This mutates the bchat manager state, so its the caller's obligation to avoid conflicts by:
    //
    // * Using a new bchat manager for each request.
    // * Pooling bchat managers.
    // * Using a single bchat manager on a single queue.
    @objc
